= Inside Out Java: Exploring Data Structures with JOL Visualization
Ivan Ponomarev
...
* Team Lead @ Synthesized
* Lecturer @ Northeastern University London
|===

== What does it take to make a lecture course?

* Prepare a 1.5-hour lecture every week for 14 consecutive weeks
* Don't feel like drawing on the board
* Want to make slides as code (https://ponomarev.uk/blog/presentation.html)
* *Is it possible for the slides to draw themselves?*

image::presentationascode.svg[{image-20-width}]


== GraphViz (Graph Visualization Software)

* Initial release: before 1991 -- more than 33 years old
* Latest release: 12.1.2, September 2024 -- still very actively developed
...
== Lightweight Java Visualizer History: 2004
...
* GNU GPL, file `LJV.java`.
* Java Reflection API + GraphViz.

== Lightweight Java Visualizer reborn: 2020

* Nuras Nogaev, Ilya Selivanov (MIPT)
* Improved API
* Updated GraphViz
* GitHub, Maven release, CI, tests
* 280+ stars on GitHub

== Java 9+: Reflection is no longer available everywhere for free
...
* We now need to add `--add-opens` directives in order to introspect classes from the standard library.
* Also, `getDeclaredFields()` does not gurantee the ordering of the fields across invocations, so we had to look for something more powerful than Reflection API.


== OpenJDK/JOL

* _JOL (Java Object Layout) is the tiny toolbox to analyze object layout schemes in JVMs. These tools are using Unsafe, JVMTI, and Serviceability Agent (SA) heavily to decode the actual object layout, footprint, and references. This makes JOL much more accurate than other tools._
* JOL is *very* low-level, it hacks into the structure of Java objects and reveals their structure bitwise in memory.
* As it's a part of OpenJDK, we can rely on its support in the future.
...
# Compressed class pointers: 0-bit shift and 0x1B6C5000000 base
...
OFF SZ                     TYPE DESCRIPTION            VALUE
  0  8                          (object header: mark)  0x0000000000000001
                                                       (non-biasable; age: 0)
  8  4                          (object header: class) 0x0008ccb8
...
== What can we learn from JOL?

* Alexey Shipilev: Java Objects Inside Out https://shipilev.net/jvm/objects-inside-out/

* Mark Word/Class Word/Field alignments -- internals of the individual object

* We are focusing on data structures layouts

image::shipilev.svg[{image-20-width}]

== 2024: LJV is now part of JOL
...
== Step 1: Creating and Configuring LJV
...
== Step 2: Creating the object to be visualized
...
* Queues
* `Map` and `ConcurrentMap`
* `NavigableMap` and `ConcurrentNavigableMap`

_Note: there will be no manually adjusted images +
in this presentation. All the diagrams are dumped "as is" +
from JOL output._
...
[source,java]
    /*0*/   x,
    /*1*/   new String(x),  //the buffer is reused
    /*2*/   new String(x.toCharArray()),
    /*3*/   x + "",
    /*4*/   "" + x,
    /*5*/   x.concat(""),   //returns x
    /*6*/   "".concat(x)

== Creation of Strings (Java 16+)
...
digraph Java {
	...
	>];
	n8 -> n3[label="value",fontsize=12];
	n1:f4 -> n8[label="4",fontsize=12];
	n1:f5 -> n2[label="5",fontsize=12];
	n9[label=<
		<table border='0' cellborder='1' cellspacing='0'>
			<tr>
	...
	>];
	n9 -> n3[label="value",fontsize=12];
	n1:f6 -> n9[label="6",fontsize=12];
}
...
* Switch to newer versions of Java, strings are getting better and better!

* In case of performance problems due to memory overhead, do string deduplication +
(but not through `intern()`).


== Further Viewing

* Alexey Shipilev
** https://www.youtube.com/watch?v=YgGAUGC9ksk[ java.lang.String Catechism]
** https://www.youtube.com/watch?v=wIyeOaitmWM[The Lord Of The Strings]
...
== Deduplication (caching) of boxed primitives
...
[source,java]
/*0*/ 42,
/*1*/ 42, // deduplicated range -128 .. 127
          // or -128 .. -XX:AutoBoxCacheMax
/*2*/ Integer.valueOf(42), //deduplicated
/*3*/ new Integer(42),   //NOT deduplicated, deprecated since Java9
/*4*/ -4242,               
/*5*/ -4242              //not deduplicated (outside of the range)


== Interim Conclusions

* Remember about deduplication of boxed primitives.
* In case of performance problems due to memory overhead, use libraries:
** https://github.com/vigna/fastutil[fastutil] 
** https://github.com/eclipse/eclipse-collections[Eclipse collections] primitive collections
** ...
** ...or wait for Project Valhalla (generic specialization).

== Our Plan

* [line-through]#Strings and Primitive Wrappers#
* *Queues*
* `Map` and `ConcurrentMap`
* `NavigableMap` and `ConcurrentNavigableMap`
...
== `PriorityQueue`: what's the trick?

* Binary min-heap, invariants:
** stem:[q[n\] \leq q[2n + 1\]]
** stem:[q[n\] \leq q[2n+2\]]
* Consequence:
** stem:[q[0\] = \min(q)]
...
* `java.util.LinkedList` is usless, use `ArrayList` / `ArrayDeque` / `PriorityQueue` instead.
* Arrays are a very powerful foundation for building efficient data structures!
* If thread safety and non-blocking behavior are needed, the idea of a linked list starts working again in `ConcurrentLinkedQueue` / `ConcurrentLinkedDeque`.
...
== 11 collisions, Java 8+: treeification
...
== LinkedHashMap: nearly ready LRU cache
...
== Collisions in MapN: linear probing
...
* Significant upgrade of HashMap and ConcurrentHashMap in Java 8.
* In Java 9 there are immutable hashmaps with open addressing.
* Don't forget about `LinkedHashMap`:
...
* [line-through]#Strings and Primitive Wrappers#
* [line-through]#Queues#
* [line-through]#`Map` and `ConcurrentMap`#
* *`NavigableMap` and `ConcurrentNavigableMap`*
...
== Red-black tree: What's the trick?
...
** the "`null` leaves" at the ends of branches are considered black.

** Both children of every red node are black.
    
** Every simple path from a parent node to a descendant leaf node has the same number of black nodes.

* Consequence:

** The path from the root to the farthest leaf (BRBRBR...) is at most twice as long as to the nearest leaf (BBB...).
...
== ConcurrentNavigableMap: the same thing, but thread-safe?

* `ConcurrentSkipListMap`
* In the comments to the source code can be found:
** two scientific papers
** book
** three PhD theses

== ConcurrentSkipListMap: the idea
[graphviz]
--
digraph G {
    rankdir = LR;
...
* Advanced algorithms work wonders.
* We have reached a point where simple visualization is not enough.

== That's all!

* Check out the project at https://github.com/openjdk/jol
* Experiment
* Send your ideas
* Source code of examples and presentation slides: +
https://github.com/inponomarev/ljv2024
